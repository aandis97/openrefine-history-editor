...
let expressionsList = listValidators;
...
$('input[type=radio][name=generate_type]').change(function() {
    generateType = $(this).val();
    if (generateType == 'mapping') {
        expressionsList = listCleanningMapping;
    } else {
        expressionsList = listValidators;
    }
...
function showExpressionList() {
    ...
    expressionsList.forEach((validator, validatorIndex) => {
...
function selectExpression(validatorIndex) {
    let validator = expressionsList[validatorIndex];
    ...
    validator.parameter.forEach(function(parameter) {
        ...
        if (parameter.type == 'textarea') {
            parametersHtml += `<textarea name="${parameter.field}" value="${parameter.value ?? ''}" data-index="${columnIndexFocused}" onchange="changeParamValue(this)" rows="5" class="form-control">${parameter.value ?? ''}</textarea><br>`;
            return;
        }
...
$('#btn-generate-json').click(function() {
    ...
    if (generateType == 'mapping') {
        columnHistories.map(function(history) {
            ...
            historyJson.push(textTransform(history.name, expression));
        });
    } else {
        historyJson.push({
        ...
        columnHistories.map(function(history) {
            ...
            historyJson.push({
                ...
                "description": "Create column "+newColumnName+" at index "+insertIndex+" based on column "+history.name+" using expression "+expression
            });
            insertIndex+=2
        });
    }

    let filename = generateType+'-history.json';
...
function generateTrim() {
    ...
    columnHistories.map(function(history) {
        historyJson.push(textTransform(history.name, "value.trim()"));
    });
...
function generateReplaceNull() {
    ...
    columnHistories.map(function(history) {
        historyJson.push(textTransform(history.name, "jython:if value.lower() == 'null' or value == '-':\n    return None\nreturn value"));
    });

    return historyJson;
}

function textTransform(columnName, expression) {
    return {
        ...
        "repeat": false,
        "repeatCount": 10,
        "description": "Text transform on cells in column "+columnName+" using expression " + expression
    };
}